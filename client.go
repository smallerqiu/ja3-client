...
func buildFromConfig(logger Logger, config *httpClientConfig) (*http.Client, BandwidthTracker, ja3.ClientProfile, error) {
	...
	if config.enabledBandwidthTracker {
		bandwidthTracker = NewTracker()
	} else {
		bandwidthTracker = NewNopeTracker()
	}

	clientProfile := config.clientProfile

	transport, err := newRoundTripper(clientProfile, config.transportOptions, config.serverNameOverwrite, config.insecureSkipVerify, config.withRandomTlsExtensionOrder, config.forceHttp1, config.withHttp3, config.certificatePins, config.badPinHandler, config.disableIPV6, config.disableIPV4, bandwidthTracker, dialer)
	if err != nil {
